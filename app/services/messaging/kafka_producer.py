...
class GeofencesKafkaProducer:
    """Producer reusable para publicar cambios de geocercas en Kafka."""

    def __init__(self) -> None:
        self.topic = settings.KAFKA_GEOFENCES_UPDATES_TOPIC
        self.brokers = [
            broker.strip()
            for broker in settings.KAFKA_BROKERS.split(",")
            if broker.strip()
    ...
    def _get_or_create(self):
        ...
        if KafkaProducer is None:
            logger.error(
                "[KAFKA GEOFENCES] Cliente kafka-python no disponible.",
                extra={
                    "extra_data": {
                        "topic": self.topic,
                        "brokers": self.brokers,
        ...
        if not self.brokers:
            logger.error(
                "[KAFKA GEOFENCES] No hay brokers configurados; se omite publicacion.",
                extra={"extra_data": {"topic": self.topic}},
            )
            return None
        ...
        try:
            ...
            logger.exception(
                "[KAFKA GEOFENCES] Error inicializando producer.",
                extra={
                    "extra_data": {
                        "topic": self.topic,
                        "brokers": self.brokers,
    ...
    def publish_update(
        ...
        try:
            ...
            logger.exception(
                "[KAFKA GEOFENCES] Error publicando evento.",
                extra={
                    "extra_data": {
                        "topic": self.topic,
                        "key": key,
    ...
    def close(self) -> None:
        ...
        try:
            ...
            logger.exception("[KAFKA GEOFENCES] Error cerrando producer.")
        finally:
            self._producer = None


class MobilityKafkaProducer:
    """Producer reusable para publicar ubicaciones de movilidad en Kafka."""

    def __init__(self) -> None:
        self.topic = settings.KAFKA_MOBILITY_TOPIC
        self.brokers = [
            broker.strip()
            for broker in settings.KAFKA_BROKERS.split(",")
            if broker.strip()
    ...
    def _get_or_create(self):
        ...
        if KafkaProducer is None:
            logger.error(
                "[KAFKA MOBILITY] Cliente kafka-python no disponible.",
                extra={
                    "extra_data": {
                        "topic": self.topic,
                        "brokers": self.brokers,
        ...
        if not self.brokers:
            logger.error(
                "[KAFKA MOBILITY] No hay brokers configurados; se omite publicacion.",
                extra={"extra_data": {"topic": self.topic}},
            )
            return None
        ...
        try:
            ...
            logger.exception(
                "[KAFKA MOBILITY] Error inicializando producer.",
                extra={
                    "extra_data": {
                        "topic": self.topic,
                        "brokers": self.brokers,
    ...
    def publish_location(
        ...
        try:
            ...
            logger.exception(
                "[KAFKA MOBILITY] Error publicando ubicacion.",
                extra={
                    "extra_data": {
                        "topic": self.topic,
                        "key": key,
...
class UnitDevicesKafkaProducer:
    """Producer para publicar cambios de asignación device→unit."""

    def __init__(self) -> None:
        self.topic = settings.KAFKA_UNIT_DEVICES_UPDATES_TOPIC
        self.brokers = [
            broker.strip()
            for broker in settings.KAFKA_BROKERS.split(",")
            if broker.strip()
    ...
    def _get_or_create(self):
        ...
        if KafkaProducer is None:
            logger.error(
                "[KAFKA UNIT DEVICES] Cliente kafka-python no disponible.",
                extra={"extra_data": {"topic": self.topic, "brokers": self.brokers}},
            )
            return None

        if not self.brokers:
            logger.error(
                "[KAFKA UNIT DEVICES] No hay brokers configurados; se omite publicacion.",
                extra={"extra_data": {"topic": self.topic}},
            )
            return None
        ...
        try:
            ...
            logger.exception(
                "[KAFKA UNIT DEVICES] Error inicializando producer.",
                extra={
                    "extra_data": {
                        "topic": self.topic,
                        "brokers": self.brokers,
    ...
    def publish_update(
        ...
        try:
            ...
            logger.exception(
                "[KAFKA UNIT DEVICES] Error publicando evento.",
                extra={
                    "extra_data": {
                        "topic": self.topic,
                        "key": key,
                        "event_type": payload.get("event_type"),
                        "device_id": (payload.get("data") or {}).get("device_id"),
                    }
                },
            )
            return False
    ...
    def close(self) -> None:
        ...
        try:
            ...
            logger.exception("[KAFKA UNIT DEVICES] Error cerrando producer.")
        finally:
            self._producer = None


class UserUnitsKafkaProducer:
    """Producer para publicar grants/revokes de user_units."""

    def __init__(self) -> None:
        self.topic = settings.KAFKA_USER_UNITS_UPDATES_TOPIC
        self.brokers = [
            broker.strip()
            for broker in settings.KAFKA_BROKERS.split(",")
            if broker.strip()
    ...
    def _get_or_create(self):
        ...
        if KafkaProducer is None:
            logger.error(
                "[KAFKA USER UNITS] Cliente kafka-python no disponible.",
                extra={"extra_data": {"topic": self.topic, "brokers": self.brokers}},
            )
            return None
        ...
        if not self.brokers:
            logger.error(
                "[KAFKA USER UNITS] No hay brokers configurados; se omite publicacion.",
                extra={"extra_data": {"topic": self.topic}},
            )
            return None
        ...
        try:
            ...
            logger.exception(
                "[KAFKA USER UNITS] Error inicializando producer.",
                extra={
                    "extra_data": {
                        "topic": self.topic,
                        "brokers": self.brokers,
    ...
    def publish_update(
        ...
        try:
            ...
            logger.exception(
                "[KAFKA USER UNITS] Error publicando evento.",
                extra={
                    "extra_data": {
                        "topic": self.topic,
                        "key": key,
                        "event_type": payload.get("event_type"),
                        "user_id": (payload.get("data") or {}).get("user_id"),
                    }
                },
            )
            return False
    ...
    def close(self) -> None:
        ...
        try:
            ...
            logger.exception("[KAFKA USER UNITS] Error cerrando producer.")
        finally:
            self._producer = None
