...
- Tabla clients: información de la organización
"""
...
def _get_billing_stats(db: Session, client_id: UUID) -> BillingStats:
    """
...
def _get_pending_amount(db: Session, client_id: UUID) -> Decimal:
    """
...
def get_billing_summary(
    ...
    active_sub = get_primary_active_subscription(db, client_id)
    ...
    # Estadísticas
    stats = _get_billing_stats(db, client_id)
    pending_amount = _get_pending_amount(db, client_id)
    
    return BillingSummaryOut(
        organization_id=client_id,
        organization_name=client.name if client else "Unknown",
        has_active_subscription=active_sub is not None,
        current_plan=current_plan,
        pending_amount=pending_amount,
        stats=stats,
        billing_email=client.billing_email if client else None
    )
...
def list_invoices(
    ...
    for i, payment in enumerate(payments):
        ...
        invoice = InvoiceOut(
            ...
            description=f"Suscripción SISCOM",
