...
def upgrade() -> None:
    ...
    op.alter_column("users", "cognito_sub", existing_type=sa.String(255), nullable=True)
    ...
    op.execute(
        "ALTER TABLE clients ADD CONSTRAINT clients_status_check CHECK (status = ANY (ARRAY['PENDING'::text, 'ACTIVE'::text, 'SUSPENDED'::text, 'DELETED'::text]))"
    )
...
def downgrade() -> None:
    ...
    op.alter_column(
        "users", "cognito_sub", existing_type=sa.String(255), nullable=False
    )

    # Revertir el constraint de status en clients
    op.execute("ALTER TABLE clients DROP CONSTRAINT IF EXISTS clients_status_check")
    op.execute(
        "ALTER TABLE clients ADD CONSTRAINT clients_status_check CHECK (status = ANY (ARRAY['ACTIVE'::text, 'SUSPENDED'::text, 'DELETED'::text]))"
    )
