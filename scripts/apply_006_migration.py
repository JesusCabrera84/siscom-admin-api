...
def apply_migration(dry_run: bool = False):
    ...
    with Session(engine) as session:
        ...
        print(
            f"   - Tabla device_events: {'✅ Existe' if has_events else '❌ No existe'}"
        )
        ...
        try:
            ...
            alembic_cfg.set_main_option(
                "script_location", str(root_dir / "app" / "db" / "migrations")
            )
        ...
        if device_count_after != device_count:
            print(
                f"⚠️  Advertencia: El número de dispositivos cambió ({device_count} → {device_count_after})"
            )
        ...
        print("   - Tabla device_events: ✅ Creada")
        print("   - Trigger de protección: ✅ Instalado")
