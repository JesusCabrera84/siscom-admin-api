...
def fix_user(email: str, password: str):
    ...
    print("=" * 80)
    print("🔧 SOLUCIONANDO PROBLEMAS DEL USUARIO EN COGNITO")
    print("=" * 80)
    print(f"Email: {email}")
    ...
    try:
        ...
        email_verified_final = next(
            (
                attr
                for attr in user_info_final.get("UserAttributes", [])
                if attr["Name"] == "email_verified"
            ),
            None,
        )
        ...
        if user_info_final["UserStatus"] == "CONFIRMED":
            ...
            print("\nPuedes probarlo con:")
            print("curl --location 'http://localhost:8000/api/v1/auth/login' \\")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print(
            "Ejemplo: python scripts/fix_cognito_user.py test4@example.com 'StringTest4*'"
        )
        sys.exit(1)
